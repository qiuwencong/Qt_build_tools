...
SET EXTRABUILDOPTIONS=-openssl-linked
SET STARTDIR=%CD%
SET SRCDIR=D:\Qt\src
SET BUILDDIR=%SRCDIR%\build
SET TOOLSDIR=%STARTDIR%\tools\
SET PREFIX=D:/Qt/installed
SET VCPKG_ROOT=D:\Qt\Tools\vcpkg-tools\vcpkg
...
SET QTURL=https://mirrors.sjtug.sjtu.edu.cn/qt/%QTRELEASE%_releases/qt/%QTMVER%/%QTVER%/submodules/qtbase-everywhere-src-%QTVER%.zip
SET QTBASEFILE=qtbase-everywhere-src-%QTVER%.zip
...
SET PATH=C:\Qt\Tools\CMake_64\bin;D:\Qt\Tools\Strawberry\perl\bin;D:\home\python\Miniconda2\envs\py38;%STARTDIR%\tools\gnuwin32\bin;%STARTDIR%\tools\jom;%STARTDIR%\tools\nasm;%VCPKG_InstalledDir%\bin;%QTINSTALLDIR%\bin;%PATH%
